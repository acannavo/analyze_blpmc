...
public:
    ...
    // CountEvents
    //
    // Counts left/right-arm coincidences passing the scintillator
    // energy cuts, using ROOT::RDataFrame. RDataFrame's Filter/Count
    // actions are automatically parallelized across worker threads
    // once ROOT::EnableImplicitMT() has been called (done once,
    // globally, in rootlogon.C) — this is what makes counting fast
    // across the many large ROOT files a full parameter scan opens.
    //
    // ALWAYS reads the full tree — every event is counted, for both
    // elastic and inelastic samples. There is no subsampling here;
    // the physical elastic/inelastic mixing ratio is applied later
    // as a weight between the two independently-measured asymmetries
    // (see the weighted combination in analyze_blpmc_scan()) rather
    // than by throwing away inelastic statistics up front. Reading
    // full statistics also sidesteps any risk of ordering bias in
    // the underlying Geant4 output (event classes are not guaranteed
    // to be uniformly distributed through a file).
    // -------------------------------------------------------
    void CountEvents(TTree* tree, Bool_t isSpinUp) {
        if (!tree) {
        ...
        Long64_t countL = *df.Filter(cutL.Data()).Count();
        Long64_t countR = *df.Filter(cutR.Data()).Count();
